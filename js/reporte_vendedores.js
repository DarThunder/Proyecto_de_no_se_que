...
async function loadEmployeeSalesReport() {
    ...
    try {
        const response = await fetch("http://localhost:8080/reports/sales-by-employee", {
            credentials: 'include'
        });
        ...
        if (employeeSales.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="5">No hay datos de ventas de empleados para mostrar.</td></tr>';
            return;
        }
