...
class CuponManager {
    constructor() {
        this.API_BASE = 'http://localhost:8080/cupones';
        this.init();
    }
    ...
    async verificarAutenticacion() {
        try {
            ...
            if (userData.role.permission_ring !== 0) {
                this.mostrarError('No tienes permisos de administrador para acceder a esta página');
                setTimeout(() => {
    ...
    async crearCupon(event) {
        ...
        const cuponData = {
            ...
            usos_maximos: formData.get('usos_maximos') ? parseInt(formData.get('usos_maximos')) : null
        };
    ...
    mostrarCupones(cupones) {
        ...
        if (cupones.length === 0) {
            cuponesList.innerHTML = '<div class="loading">No hay cupones creados</div>';
            return;
        }

        cuponesList.innerHTML = cupones.map(cupon => `
            <div class="cupon-card">
                <div class="cupon-header">
                    <span class="cupon-codigo">${cupon.codigo}</span>
                    <span class="cupon-descuento">${cupon.descuento}% OFF</span>
                </div>
                <div class="cupon-nombre">${cupon.nombre}</div>
                <div class="cupon-info">
                    <span>Creado: ${new Date(cupon.createdAt).toLocaleDateString()}</span>
                    <span>Usos: ${cupon.usos_actuales}${cupon.usos_maximos ? `/${cupon.usos_maximos}` : ''}</span>
                </div>
                <div class="cupon-info">
                    <span>Estado: ${cupon.activo ? 'Activo' : 'Inactivo'}</span>
                    ${cupon.fecha_expiracion ? 
                        `<span>Expira: ${new Date(cupon.fecha_expiracion).toLocaleDateString()}</span>` : 
                        '<span>Sin expiración</span>'
                    }
                </div>
                <div class="cupon-actions">
                    <button class="btn-danger" onclick="cuponManager.eliminarCupon('${cupon._id}')">
                        <i class='bx bx-trash'></i> Eliminar
                    </button>
                    <button class="btn-secondary" onclick="cuponManager.toggleActivo('${cupon._id}', ${!cupon.activo})">
    ...
    async toggleActivo(id, nuevoEstado) {
        try {
            ...
            this.mostrarMensaje(`Cupón ${nuevoEstado ? 'activado' : 'desactivado'} exitosamente`, 'success');
            await this.cargarCupones();
