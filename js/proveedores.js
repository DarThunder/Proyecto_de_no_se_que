/**
 * @file js/proveedores.js
 * @description Gestiona el mantenimiento de proveedores en el sistema.
 * Permite listar, crear, editar y eliminar proveedores, así como gestionar su estado (activo/inactivo).
 * Incluye validación de permisos de usuario (Admin/Gerente) al inicio.
 */

// --- Estado Global ---

/**
 * Indica si el formulario se encuentra en modo de edición (`true`) o creación (`false`).
 * @type {boolean}
 */
let isEditing = false;

/**
 * Almacena el ID del proveedor que se está editando actualmente.
 * Es `null` durante la creación de un nuevo registro.
 * @type {string|null}
 */
...
/** @type {HTMLElement} Título del modal. */
const modalTitle = document.getElementById("modal-title");
/** @type {HTMLFormElement} Formulario de datos del proveedor. */
const providerForm = document.getElementById("provider-form");
/** @type {HTMLElement} Cuerpo de la tabla donde se listan los proveedores. */
...
/**
 * Inicializa la gestión de proveedores cuando el DOM está listo.
 * 1. Verifica la sesión y permisos del usuario.
 * 2. Carga la lista de proveedores.
 * 3. Configura los listeners de la interfaz.
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (providerForm) providerForm.addEventListener("submit", handleFormSubmit);
});
...
/**
 * Obtiene la lista de proveedores desde la API y renderiza la tabla.
 * @async
 */
...
/**
 * Prepara la interfaz para registrar un nuevo proveedor.
 * Resetea el formulario y establece el estado por defecto.
 */
 ...
 * Carga los datos actuales desde la API y rellena los campos del formulario.
 * @async
 * @param {string} id - ID del proveedor a editar.
 */
async function openEditModal(id) {
  ...
  try {
    ...
    document.getElementById("provider-contactName").value =
 ...
 * Recolecta los datos y realiza la petición correspondiente (POST o PUT).
 * @async
...
async function handleFormSubmit(e) {
  ...
  try {
    ...
    // Éxito
    closeProviderModal();
    await loadProviders(); // Recargar tabla
  } catch (error) {
    console.error(error.message);
...
// Exponer funciones globales para los onclicks dinámicos
