...
/**
 * Genera el reporte de productos más vendidos (Top 10).
 * Utiliza un Aggregation Pipeline para:
 * 1. Desglosar los ítems de cada venta.
 * 2. Agrupar por variante y sumar cantidades/ingresos.
 * 3. Unir con las colecciones de Variantes y Productos para obtener detalles.
 *
 * @route GET /reports/bestsellers
...
router.get("/bestsellers", verifyToken, hasPermission(1), async (req, res) => {
  try {
    const bestSellers = await Sale.aggregate([
      { $unwind: "$items" }, // Separa cada item del array en un documento propio
      {
        $group: {
          _id: "$items.variant",
      ...
      { $sort: { totalQuantitySold: -1 } }, // Orden descendente
      { $limit: 10 },
      {
        $lookup: {
...
/**
 * Compara el rendimiento de ventas por canal (Web vs POS).
 *
 * @route GET /reports/sales-by-channel
 * @access Private (Ring 1 - Manager)
 * @returns {object} { online: {revenue, salesCount}, pos: {revenue, salesCount} }
 */
router.get(
  "/sales-by-channel",
  verifyToken,
...
/**
 * Reporte de rendimiento por empleado (Cajeros).
 * Calcula ventas totales y ticket promedio.
 * Filtra solo ventas POS con cajero asignado.
 *
 * @route GET /reports/sales-by-employee
 * @access Private (Ring 1 - Manager)
 */
router.get(
  "/sales-by-employee",
